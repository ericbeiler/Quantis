...
namespace Visavi.Quantis
{
    public class EquityContainerService
    {
        ...
        public async Task<int> ProcessWorkingEquityFile([BlobTrigger(EquityArchivesContainerName + "/workingset/{name}", Connection = "QuantisStorageConnection")] Stream stream, string name, FunctionContext context)
        {
            ...
            try
            {
                ...
                if (stream.Length > 0)
                {
                    ...
                    await containerClient.DeleteBlobIfExistsAsync("/workingset/" + name);
                }
                else
                {
